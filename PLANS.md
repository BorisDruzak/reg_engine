...
- 2026-07-17 notification and searchable-choice overlays are corrected
  ...
  panel is layered above tag search controls and has a bounded scrollable
  notification body with a fixed header. Open searchable-choice pickers use a
  higher shared layer than adjacent card fields without changing card geometry.
  New regression coverage proves outside/Escape dismissal and the active picker
...
- 2026-07-15 card-creation preview now mirrors the selected template's saved
  ...
  from that same sequence, and keeps unplaced template fields after the
  layout-defined fields using the schema fallback. The creation presentation
...
- 2026-07-14 organization and link workflow refinement is released at
  ...
  `Скрывать колонку «Организация»` switch; import still asks for one target
...
- 2026-07-14 readable card-list reference values are released at `d7bd35d7`.
  ...
  matching PostgreSQL API regression scenario passed against the disposable
  `reg_engine_migration_test` database. Scoped Ruff, mypy (88 source files),
  frontend TypeScript, the focused frontend regression test, and the Vite
  production build pass; ESLint retains the existing non-blocking
  `FilledCardLayout.tsx` hook-dependency warning. The server checkout and
  frontend assets are deployed, the service and health endpoint are active,
  and the browser now shows `Территориальное управление №3` and
  `Управление Образования` instead of their stored UUIDs. No card data was
  changed during browser verification.
