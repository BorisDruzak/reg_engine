...
- Runtime server: configured by local environment or ignored script config.
- Server checkout: configured by local environment or ignored script config.
- Main database: PostgreSQL endpoint configured by local environment or ignored script config.

## Selected Approach

Use PowerShell scripts as the main user interface because the primary workflow runs from the Codex Windows app. Server work is executed through the configured SSH target, keeping Linux commands close to the deploy script without requiring the user to switch terminals.
...
- `scripts/deploy.ps1` updates the configured server checkout from GitHub and runs server checks.
