...
| Registries | Create/list/read/schema API exists: `GET/POST /api/v1/registries`, `GET /api/v1/registries/{registry_id}`, `GET /api/v1/registries/{registry_id}/schema`. No registry metadata update, archive endpoint, or archived read scope exists. | Registry/schema read UI exists in `frontend/src/features/registry/RegistriesAndSchema.tsx`; no create/update/archive controls. | Backend gap in Phase 2K.2; create UI can start in Phase 2L.4 before update/archive. |
