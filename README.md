...
schema properties including scalar `enum` values, `oneOf` option titles, and
`format: "date"` string inputs, using template default parameter JSON for
report runs when manual run JSON is empty, choosing or editing JSON, CSV, XLSX,
or PDF output,
