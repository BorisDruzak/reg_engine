...
All initial read-only tools are marked with `readOnlyHint=true` and use HTTP
`GET` only. MCP API requests send `X-Reg-Engine-Source: mcp`; user audit events
created through the API can therefore use `source=mcp`. Direct database access,
...
- No MCP report-output/generated-document-content/binary-document/public-link tools beyond the explicitly
