...
- XLSX import/export, non-JSON report outputs, remaining report polish, and MCP write tools are later phases.
...
report types in this first backend slice are `registry_cards`, `card_detail`,
and `period_summary`; supported output format is `json`.
...
creating, updating, and archiving report templates, generating JSON report
runs, downloading generated report content, and archiving report runs through
the existing report API.
...
- No non-JSON report output, report scheduling, report charts, or visual report
  builder yet.
