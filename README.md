...
- XLSX card import/export is available as a row-oriented technical exchange
  format. XLSX report output is available for existing report types. PDF
  report outputs, additional report polish, and MCP write
...
supported output formats are `json`, `csv`, and `xlsx`.
...
creating, updating, and archiving report templates, choosing JSON, CSV, or XLSX
...
PDF report outputs, scheduling, charts, public-link report workflows,
...
- No PDF report output, report scheduling, report charts, or visual report
  builder yet.
